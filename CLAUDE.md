...
| **LokrLab** | Evolving (0.12.113) | Suite: host + Character + Ability + Encounter; File → Edit Vanilla Hero (slug_token folder + Model-prefab combat rig); Close Lab flushes Description loc and always reloads content; Sandbox Stop restores camera ortho; File → Save / Ctrl+S / dirty `*` / close prompt; Animator feel; Ability overhaul; hover coverage; Encounter visual catalogues + camera bounds; Setup auto-load + Restart board + catalogue drag-to-place; prop snap or free-move. [Encounter Creator](docs/roadmaps/started/encounter-creator.md) Phases 1-16b all confirmed in-game as of 2026-08-16. Phase 17: one Sandbox workspace; Sandbox Level is always 1-3. [Vanilla Encounter Edit](docs/roadmaps/started/vanilla-encounter-edit.md) Phase 1 import spike: floor/walkable/camera bounds fixed and confirmed in-game 2026-08-17; found and fixed host double-spawn, then a prop-loading bug it unmasked (prefabReference.name now used instead of the unreliable prefabName string); re-confirm pending |
...
**Last updated**: 2026-08-17 (Fix vanilla prop import not resolving to a loadable prefab 0.12.113)
